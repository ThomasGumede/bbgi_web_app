...
def search(request):
    ...
    place = request.GET.get("province", None)
    if not query:
    ...
    results_dic = {
        ...
        "listings": Business.objects.filter(Q(title__icontains=query) | Q(main_address__icontains=place or query) | Q(bbbee_level__icontains=query) | Q(province__icontains=place or query)),
    }
    context = {}
