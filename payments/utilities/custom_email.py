...
def send_tickets_email(status, order: TicketOrderModel, request):
    try:
        ...
        sent = send_html_email_with_attachments(order.email, mail_subject, message, "BBGI Events <noreply@bbgi.co.za>", files)
        sent = send_html_email_with_attachments('gumedethomas12@gmail.com', mail_subject, message, "BBGI Events <noreply@bbgi.co.za>", files)
        if not sent:
...
def send_subscription_order_received_to_admin(order: SubscriptionOrder, request):
    try:
        ...
        if not sent:
            email_logger.error(f"Failed to send subscription email to 'gumedethomas12@gmail.com', 'finance@bbgi.co.za', 'listings@bbgi.co.za' for order number {order.order_number}")
            return False
