...
def subscription_payment_success(request, subscription_id):
    ...
    try:
        ...
        if updated:
            ...
            check_payment_update_2_subscription.apply_async((subscription.checkout_id, domain, protocol))
            logger.info(f"something went wrong: check_payment_update_2_subscription.apply_async((subscription.checkout_id, domain, protocol))")

    except PaymentInformation.DoesNotExist as ex:
        check_payment_update_2_subscription.apply_async((subscription.checkout_id, protocol, domain))
